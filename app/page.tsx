      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <Card className="w-full max-w-[400px] backdrop-blur-[2px] bg-gradient-to-r from-white to-gray-100 border border-[#eaeaea] dark:bg-[linear-gradient(110deg,#333_0.6%,#222)] dark:border-neutral-600">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-center">Sign in to manage your recruitment process</CardDescription>
          </CardHeader>

          <CardContent className="space-y-4 pt-4">
            {/* Loading Step Indicator */}
            {isLoading && loginStep && (
              <div className="flex items-center justify-center p-3 rounded-md bg-primary/10 border border-primary/20">
                <Loader2 className="h-4 w-4 animate-spin mr-2 text-primary" />
                <span className="text-sm text-primary font-medium">{loginStep}</span>
              </div>
            )}
